...
app.use(express.static("src/asset/upload"));
...
app.use("/debug", async (req, res) => {
  return res.status(401).json("Unauthorizedss");
...
app.use("/pdf", async (req, res) => {
  ...
  QRCode.toDataURL(
    ...
    function (_, qr) {
      ...
      const car = path.join(__dirname, "asset/upload", "car.jpg");
      const car2 = path.join(__dirname, "asset/upload", "car2.jpg");
      const stamp = path.join(__dirname, "asset/upload", "stempel.png");
      const signature = path.join(__dirname, "asset/upload", "ttd.png");
      const logo = path.join(__dirname, "asset/image", "logo.png");
      ...
      pdf.image(stamp, 450, 711, { width: 45 }); //PETUGAS
      pdf.image(signature, 480, 725, { width: 25 });

      pdf.image(stamp, 480, 779, { width: 45 }); //AGENCY
      pdf.image(signature, 510, 793, { width: 25 });
      ...
      pdf.image(car, 132, 395, { width: 70, height: 40 });
      pdf.image(car2, 222.01, 395, { width: 70, height: 40 });
      pdf.image(car, 312.03, 395, { width: 70, height: 40 });
      pdf.image(car2, 402.05, 395, { width: 70, height: 40 });
      ...
      pdf.fontSize(6).font("Serif-Bold").text(": LULUS UJI BERKALA", 414, 637);
